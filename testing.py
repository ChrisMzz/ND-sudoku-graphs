...
def make_cmap(colouring:dict, sqn:int, cmap_str:str = 'Spectral'):
    ...
    return [cmap(k/digits) for k in range(digits)] + [(0,0,0,0)]*int(abs(maxcol+1-digits)), cmap_str
    ...
    if N == 3:
        ...
        if voxels_on:
            for i,p in enumerate(POS):
                ...
                ax.voxels(*(np.indices((digits+1,digits+1,digits+1))-1/2), vox, color=tuple(colouring[i][:3])+(0.05,))
